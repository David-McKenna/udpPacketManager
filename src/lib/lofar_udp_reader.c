...
int lofar_udp_parse_headers(lofar_udp_meta *meta, char header[MAX_NUM_PORTS][UDPHDRLEN], const int beamletLimits[2]) {
	...
	for (int port = 0; port < meta->numPorts; port++) {
		...
		if (beamletLimits[1] != 0 && (beamletLimits[1] < (meta->portCumulativeBeamlets[port] + meta->portBeamlets[port])) && (beamletLimits[1] >= meta->portCumulativeBeamlets[port])) {
			meta->portBeamlets[port] = beamletLimits[1] - meta->portCumulativeBeamlets[port] - beamletLimits[0] + 1;
		}
...
int lofar_udp_setup_processing(lofar_udp_meta *meta) {
	...
	if (equalIO) {
		...
		for (int port = 0; port < meta->numPorts; port++) {
			workingData += hdrOffset + UDPHDRLEN + (meta->portBeamlets[port] - meta->baseBeamlets[port]) * UDPNPOL * ((float) meta->inputBitMode / 8.0) * UDPNTIMESLICE;
			printf("%d %d\n", workingData, meta->portBeamlets[port]);
		}

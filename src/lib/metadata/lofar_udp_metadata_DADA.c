...
int64_t _lofar_udp_metadata_write_DADA(const lofar_udp_metadata *hdr, int8_t *const headerBuffer, int64_t headerLength) {
	...
	returnVal += _writeDouble_DADA(headerBuffer, "HDR_VERSION", hdr->hdr_version, 0);
	// Lovely chicken and the egg problem, we'll update it again later.
	returnVal += _writeLong_DADA(headerBuffer, "HDR_SIZE", (long) strnlen(headerBuffer, headerLength));
	returnVal += _writeStr_DADA(headerBuffer, "INSTRUMENT", "CASPSR"); // TODO: ANYTHING ELSE, psrchive exits early if the machine isn't recognised.
	returnVal += _writeStr_DADA(headerBuffer, "TELESCOPE", hdr->telescope);
	returnVal += _writeInt_DADA(headerBuffer, "TELESCOPE_RSP", hdr->telescope_rsp_id);
	returnVal += _writeStr_DADA(headerBuffer, "RECEIVER", hdr->receiver);
	returnVal += _writeStr_DADA(headerBuffer, "OBSERVER", hdr->observer);
	returnVal += _writeStr_DADA(headerBuffer, "HOSTNAME", hdr->hostname);
	returnVal += _writeInt_DADA(headerBuffer, "BASEPORT", hdr->baseport);
	returnVal += _writeInt_DADA(headerBuffer, "FILE_NUMBER", hdr->output_file_number / hdr->upm_num_inputs);
	returnVal += _writeInt_DADA(headerBuffer, "RAW_FILE_NUMBER", hdr->output_file_number);
	...
	for (int port = 0; port < hdr->upm_num_inputs; port++) {
		...
		returnVal += _writeStr_DADA(headerBuffer, keyfmt, hdr->rawfile[port]);
	}

	returnVal += _writeStr_DADA(headerBuffer, "SOURCE", hdr->source);
	returnVal += _writeStr_DADA(headerBuffer, "COORD_BASIS", hdr->coord_basis);
	returnVal += _writeStr_DADA(headerBuffer, "RA", hdr->ra);
	returnVal += _writeStr_DADA(headerBuffer, "DEC", hdr->dec);
	returnVal += _writeStr_DADA(headerBuffer, "RA_ANALOG", hdr->ra_analog);
	returnVal += _writeStr_DADA(headerBuffer, "DEC_ANALOG", hdr->dec_analog);
	returnVal += _writeDouble_DADA(headerBuffer, "RA_RAD", hdr->ra_rad, 0);
	returnVal += _writeDouble_DADA(headerBuffer, "DEC_RAD", hdr->dec_rad, 0);
	returnVal += _writeDouble_DADA(headerBuffer, "RA_RAD_ANALOG", hdr->ra_rad_analog, 0);
	returnVal += _writeDouble_DADA(headerBuffer, "DEC_RAD_ANALOG", hdr->dec_rad_analog, 0);
	returnVal += _writeStr_DADA(headerBuffer, "OBS_ID", hdr->obs_id);
	...
	returnVal += _writeStr_DADA(headerBuffer, "UTC_START", tmpUtc);

	returnVal += _writeDouble_DADA(headerBuffer, "MJD_START", hdr->obs_mjd_start, 0);
	returnVal += _writeLong_DADA(headerBuffer, "OBS_OFFSET", hdr->obs_offset);
	returnVal += _writeLong_DADA(headerBuffer, "OBS_OVERLAP", hdr->obs_overlap);
	returnVal += _writeStr_DADA(headerBuffer, "BASIS", hdr->basis);
	returnVal += _writeStr_DADA(headerBuffer, "MODE", hdr->mode);


	returnVal += _writeDouble_DADA(headerBuffer, "FREQ", hdr->freq_raw, 0);
	returnVal += _writeDouble_DADA(headerBuffer, "BW", hdr->bw, 1);
	returnVal += _writeDouble_DADA(headerBuffer, "CHAN_BW", hdr->subband_bw, 1);
	returnVal += _writeDouble_DADA(headerBuffer, "FTOP", hdr->ftop, 0);
	returnVal += _writeDouble_DADA(headerBuffer, "FBOT", hdr->fbottom, 0);
	returnVal += _writeInt_DADA(headerBuffer, "NCHAN", hdr->nchan);
	returnVal += _writeInt_DADA(headerBuffer, "NRCU", hdr->nrcu);
	returnVal += _writeInt_DADA(headerBuffer, "NPOL", hdr->npol);
	// DSPSR front end: -32 is a float, dspsr backend: -32 is a very large number because this is stored in an unsigned integer.
	// Why?
	returnVal += _writeInt_DADA(headerBuffer, "NBIT", hdr->nbit > 0 ? hdr->nbit : -1 * hdr->nbit);
	returnVal += _writeInt_DADA(headerBuffer, "RESOLUTION", hdr->resolution);
	returnVal += _writeInt_DADA(headerBuffer, "NDIM", hdr->ndim);
	returnVal += _writeDouble_DADA(headerBuffer, "TSAMP", hdr->tsamp * 1e6, 0);
	returnVal += _writeStr_DADA(headerBuffer, "STATE", hdr->state);


	returnVal += _writeStr_DADA(headerBuffer, "COMMENT", "Further contents are metadata from UPM");

	returnVal += _writeStr_DADA(headerBuffer, "UPM_VERSION", hdr->upm_version);
	returnVal += _writeStr_DADA(headerBuffer, "UPM_REC_VERSION", hdr->rec_version);
	returnVal += _writeStr_DADA(headerBuffer, "UPM_DAQ", hdr->upm_daq);
	returnVal += _writeStr_DADA(headerBuffer, "UPM_BEAMCTL", hdr->upm_beamctl);

	if (snprintf(keyfmt, 15, "UPM_OUTPFMT%d", fileoutp) < 0) {
		fprintf(stderr, "ERROR %s: Failed to create key for UPM_OUTPFMT / %d, exiting.\n", __func__, fileoutp);
		return -1;
	}
	returnVal += _writeStr_DADA(headerBuffer, keyfmt, hdr->upm_outputfmt[fileoutp]);

	returnVal += _writeStr_DADA(headerBuffer, "UPM_OUTPFMTCMT", hdr->upm_outputfmt_comment);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_NUM_INPUTS", hdr->upm_num_inputs);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_READER", hdr->upm_reader);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_MODE", hdr->upm_procmode);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_BANDFLIP", hdr->upm_bandflip);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_REPLAY", hdr->upm_replay);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_CALIBRATED", hdr->upm_calibrated);
	returnVal += _writeLong_DADA(headerBuffer, "UPM_BLOCKSIZE", hdr->upm_blocksize);
	returnVal += _writeLong_DADA(headerBuffer, "UPM_PKTPERITR", hdr->upm_pack_per_iter);
	returnVal += _writeLong_DADA(headerBuffer, "UPM_PROCPKT", hdr->upm_processed_packets);
	returnVal += _writeLong_DADA(headerBuffer, "UPM_DRPPKT", hdr->upm_dropped_packets);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_LSTDRPPKT", hdr->upm_last_dropped_packets);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_BITMODE", hdr->upm_input_bitmode);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_RCUCLOCK", hdr->upm_rcuclock);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_RAWBEAMLETS", hdr->upm_rawbeamlets);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_UPPERBEAMLET", hdr->upm_upperbeam);
	returnVal += _writeInt_DADA(headerBuffer, "UPM_LOWERBEAMLET", hdr->upm_lowerbeam);
